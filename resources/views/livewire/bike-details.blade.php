<div>
            ...
            <div class="p-4 shadow-md shadow-black">
                ...
                <div class="flex flex-row justify-center items-center gap-4 m-5 shadow-sm shadow-gray-800 rounded-md py-8">
                    ...
                    <div>
                        ...
                        <div class="flex flex-row gap-4 justify-center">
                            <div>
                                <p>From date</p>
                                <input type="date" min="{{ date('Y-m-d') }}" class="w-[100%] bg-slate-200 rounded" wire:model.live="from_date"
                                    @if (session()->get('from_date')) value="{{ $from_date }}" readonly @endif>
                            </div>
                            <div>
                                <p>To date</p>
                                <input type="date" class="w-[100%] bg-slate-200 rounded" wire:model.live="to_date"
                        ...
                        @if ($dateerror)
                            <div class="flex flex-row justify-center py-1">
                                <span class="text-red-600 text-sm">{{ $dateerror }}</span>
                            </div>
                        @endif
                    </div>
                        ...
                        <div class="font-normal grid grid-cols-2 gap-6 text-md">
                            ...
                            <div>
                                <ul>
                                    ...
                                    <li>{{ $bike->number_plate }}</li>
                                    <li>
                                        View <button class="bg-gray-900 text-white py-0.5 px-3 text-sm rounded" wire:click="billbookdialog">Click me</button>
                                    </li>
                                </ul>
                            </div>
                        </div>
                ...
                <div class="w-screen h-screen {{ $billbookdisplay }} top-0 right-0 backdrop-blur-sm bg-opacity-70 bg-gray-300 flex justify-center items-center">
                    <div class="bg-white">
                        <div class="bg-white rounded relative p-2">
                            <button type="button" wire:click="billbookdialog(2)" class="absolute top-2 right-2">
                                <i class="fa fa-times hover:bg-black hover:text-white rounded-full p-1" aria-hidden="true"></i>
                            </button>
                        ...
                        <div class="bg-white p-5 rounded-lg shadow-lg">
                            <div class="flex items-center justify-center mb-3">
                                <svg class="animate-spin h-6 w-6 text-blue-500 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    ...
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.843 3.029 7.971l2.03-2.03zM12 20a8 8 0 100-16 8 8 0 000 16z"></path>
                                </svg>
                                <span class="text-blue-500 font-semibold">Please wait...</span>
                            </div>
                            <p class="text-gray-600">Rental Process is Processing...</p>
                        </div>
                    </div>
                @endif
                        ...
                        <div class="flex flex-row text-sm justify-center gap-4">
                            @forelse ($recommendedbikes as $rbike)
                                <div class="flex flex-row gap-3 border-2 w-60 rounded">
                                    <div>
                                        <img src="{{ asset('storage/variant_images/' . $rbike->variant->variant_image) }}" alt="image" class="w-[100px] h-[180px] object-contain">
                                    </div>
                                    <div>
                                        <ul>
                                            <li>{{ $rbike->variant->brand->brand_name }}</li>
                                            <li>{{ $rbike->variant->variant_name }}</li>
                                            <li>Rental Price: {{ $rbike->variant->variant_rental_price }}</li>
                                            <li>cc: {{ $rbike->cc }}</li>
                                        </ul>
                                        <a href="{{ route('renter.bikedetails', $rbike->id) }}" class="bg-slate-600 text-white hover:bg-black min-w-full px-3 rounded-sm">View Details</a>
                                    </div>
                                </div>
                                @if ($loop->iteration == 3)
                                    @break
                                @endif
                            @empty
                                <h2 class="text-xl font-mono text-center pb-3">No Bikes Available</h2>
                            @endforelse
                        </div>
                    </div>
                @endif
                ...
                @if ($toogledialog == 'show')
                    <div class="fixed top-0 h-screen w-screen backdrop-blur-[10px] flex justify-center items-center" id="resultdialog">
                        <div class="bg-white inline-block p-6 rounded relative shadow-2xl hover:scale-105">
                            <button class="absolute top-2 right-4" wire:click="toogle()" title="close dialog">
                                <i class="fas fa-times scale-110 rounded"></i>
                            </button>
                            ...
                            <div class="flex flex-row justify-center gap-3 py-1">
                                <button class="bg-slate-800 hover:bg-black text-sm px-3 py-0.5 rounded-sm text-white" wire:click="checkoutRent">Cash on Hand <small>(Credit)</small></button>
                                <button class="bg-violet-950 hover:bg-violet-500 text-sm px-3 py-0.5 rounded-sm text-white" id="payment-button">Pay with Khalti</button>
                            </div>
    ...
    const btn = document.getElementById("khalti-payment-button");
    btn.onclick = function() {
        // minimum transaction amount must be 1000 paisa (i.e., 10)
        console.log({{ $total_rental_price * 100 }});
        checkout.show({ amount: {{ $total_rental_price * 100 }} });
    };
});
