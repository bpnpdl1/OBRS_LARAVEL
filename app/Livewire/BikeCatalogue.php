...
class BikeCatalogue extends Component
{
    ...
    {
        $bikesArray = $bikes->all(); // Keeping the result as an Eloquent Collection
    ...
    {
        $pivot = $bikes[$high]->variant_rental_price; // Accessing object property
        $i = ($low - 1);

        for ($j = $low; $j < $high; $j++) {
            if ($bikes[$j]->variant_rental_price <= $pivot) { // Object access
                $i++;
                $temp = $bikes[$i];
                $bikes[$i] = $bikes[$j];
                $bikes[$j] = $temp;
    ...
    private function filterBikesByCC($bikes, $ccValue)
    {
        ...
        while ($low <= $high) {
            ...
            if ($bikes[$mid]->cc <= $ccValue) {
                $filtered->push($bikes[$mid]);
